    ...
    <div x-data="{ pokedyes: false, pokedno: false, open: true }" x-show="open" 
        ...
        <div x-on:click.away="open = false"
            ...
            <button x-on:click="open = false" {{ session('alert.blue-action') }}
                class="bg-blue-200 py-1 m-auto border border-blue-800 rounded-full text-blue-800 bg-gradient-to-tl from-blue-300 via-transparent to-transparent"
                :class="pokedyes
                    ? 'px-10 shadow-none'
                    : 'px-11 shadow-lg'"
            ...
            <button x-on:click="open = false" {{ session('alert.red-action') }}
                class="bg-red-200 py-1 m-auto border border-red-800 rounded-full text-red-800 bg-gradient-to-tl from-red-300 via-transparent to-transparent"
                :class="pokedno
                    ? 'px-10 shadow-none'
                    : 'px-11 shadow-lg'"
